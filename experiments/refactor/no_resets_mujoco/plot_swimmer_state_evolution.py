...
for run in range(10):
    reward_rate_no_resets.append(np.load(
        os.path.expanduser(f"~/mydata/out/refactor/no_resets_mujoco/{168*run + 3}_average_reward.npy")
    ))
    reward_rate_resets.append(np.load(
        os.path.expanduser(f"~/mydata/out/refactor/no_resets_mujoco/{168*run + 59}_eval_average_reward.npy")
    ))
print("no resets")
for run in range(10):
...
reward_rate_no_resets_sample_run = np.load(
    os.path.expanduser("~/mydata/out/refactor/no_resets_mujoco/171_visited_observations.npy")
)
print("plot reset w.p. 0.001, discount factor 0.999, run 0")
reward_rate_resets_sample_run = np.load(
    os.path.expanduser("~/mydata/out/refactor/no_resets_mujoco/59_visited_observations.npy")
)
...
for i in range(4):
    ...
    os.makedirs(os.path.expanduser(f"~/pearl_execution/pearl/experiments/refactor/no_resets_mujoco/swimmer_state_evolution"), exist_ok=True)
    plt.savefig(os.path.expanduser(f"~/pearl_execution/pearl/experiments/refactor/no_resets_mujoco/swimmer_state_evolution/swimmer_{i}.pdf"), bbox_inches='tight', pad_inches=0.4)
    plt.close()
