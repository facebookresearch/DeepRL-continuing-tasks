    ...
    for task in tasks:
        ...
        for algo in algorithms:
            if task in my_data[algo]:
                ...
                d = {
                    "10": (rwd_10_val, rwd_10_ste, eps_rwd_10_val, eps_rwd_10_ste),
                    "100": (rwd_100_val, rwd_100_ste, eps_rwd_100_val, eps_rwd_100_ste),
                    "1000": (
                        rwd_1000_val,
                        rwd_1000_ste,
                        eps_rwd_1000_val,
                ...
                sig = stats_significant(
                    ...
                    d[eval_on_x_reset_cost][2],
                    d[eval_on_x_reset_cost][3],
                )
                if (sig[0] or sig[1]) and d[eval_on_x_reset_cost][0] >= d[
                    eval_on_x_reset_cost
                ][2]:
                    row += f"\\textcolor{{gray}}{{{d[eval_on_x_reset_cost][2]:.1f} $\pm$ {d[eval_on_x_reset_cost][3]:.1f}}}"
                elif (sig[0] or sig[1]) and d[eval_on_x_reset_cost][0] < d[
                    eval_on_x_reset_cost
                ][2]:
                    row += f"\\textbf{{{d[eval_on_x_reset_cost][2]:.1f} $\pm$ {d[eval_on_x_reset_cost][3]:.1f}}}"
                else:
                    row += f"{d[eval_on_x_reset_cost][2]:.1f} $\pm$ {d[eval_on_x_reset_cost][3]:.1f}"
                row += " & "
        row = row[:-2] + r"\\ " + "\n"
        latex_code += row
