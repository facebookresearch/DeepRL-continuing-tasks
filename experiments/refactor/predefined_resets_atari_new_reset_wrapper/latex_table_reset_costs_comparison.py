...
def generate_latex_table(my_data):
    ...
    latex_code += (
        r"& Reset cost & "
        + " & ".join([f"1 & 10 & 100" for algo in algorithms])
        + r" \\"
        + "\n"
    )
    latex_code += r"\midrule" + "\n"
    latex_code += (
        r"\multirow{5}{*}{\makecell{Reward \\ rate \\ w/o \\ reset \\ cost}}" + "\n"
    )
    for task in tasks:
    ...
    for task in tasks:
        ...
        for algo in algorithms:
            if task in my_data[algo]:
                ...
                tmp = [
                    (rst_10_val, rst_10_ste),
                    (rst_100_val, rst_100_ste),
                    (rst_1000_val, rst_1000_ste),
                ]
                sig = stats_significant(
                    rst_10_val,
                    rst_10_ste,
