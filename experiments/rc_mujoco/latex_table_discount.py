...
data = {algo: {"RC 0.999": {}, "RC 1.0": {}, "0.999": {}} for algo in algorithms}
for match in matches:
    _, task, algo, val0, ste0, val1, ste1, val2, ste2, val3, ste3, val4, ste4 = match
    if algo in algorithms and task in tasks:
        if float(val0) - random_average_reward[task] < 0:
            data[algo]["RC 0.999"][task] = (None, None)
            data[algo]["RC 1.0"][task] = (None, None)
        else:
            data[algo]["RC 0.999"][task] = (
                (float(val1) - random_average_reward[task])
                / (float(val0) - random_average_reward[task])
                - 1,
                stats_significant(float(val1), float(ste1), float(val0), float(ste0)),
            )
            data[algo]["RC 1.0"][task] = (
                (float(val2) - random_average_reward[task])
                / (float(val0) - random_average_reward[task])
                - 1,
                stats_significant(float(val2), float(ste2), float(val0), float(ste0)),
            )
        if float(val3) - random_average_reward[task] < 0:
            data[algo]["0.999"][task] = (None, None)
        else:
            data[algo]["0.999"][task] = (
...
def generate_latex_table(data):
    ...
    latex_code += (
        r"& Use TD-based RC & "
        + " & ".join([f" Y & Y & N " for algo in algorithms])
        + r" \\"
        + "\n"
    )
    latex_code += (
        r"& Discount factor & "
        + " & ".join([f" 0.999 & 1.0 & 0.999 " for algo in algorithms])
        + r" \\"
        + "\n"
    )
    ...
    for task in tasks:
        ...
        for algo in algorithms:
            if algo in data and task in data[algo]["RC 0.999"]:
                rc_val, sig = data[algo]["RC 0.999"][task]
                rc_str = f"{rc_val * 100:.2f}" if rc_val is not None else "N/A"
                if sig == False:
                    row += f"\\textcolor{{gray}}{{{rc_str}}} &"
                else:
                    row += f"{rc_str} & "
            if algo in data and task in data[algo]["RC 1.0"]:
                rc_val, sig = data[algo]["RC 1.0"][task]
                rc_str = f"{rc_val * 100:.2f}" if rc_val is not None else "N/A"
                if sig == False:
                    row += f"\\textcolor{{gray}}{{{rc_str}}} &"
                else:
                    row += f"{rc_str} & "
            if algo in data and task in data[algo]["0.999"]:
                rc_val, sig = data[algo]["0.999"][task]
                rc_str = f"{rc_val * 100:.2f}" if rc_val is not None else "N/A"
                if sig == False:
                    row += f"\\textcolor{{gray}}{{{rc_str}}} &"
                else:
    ...
    latex_code += r"}" + "\n"
    latex_code += (
        r"\caption{TD-based reward centering is less sensitive to the choice of the discount factor than noncentered methods.} " + "\n"
    )
    latex_code += r"\label{tab: sensitivity to the discount factor mujoco} " + "\n"
